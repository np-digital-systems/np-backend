...
/**
 * One costing that spends on a given head, for changing them all at once.
 *
 * A rate the temple pays — the saathupadi's salary, the melam's fee — is one
 * decision that lands on a dozen costings. Answering it a dozen times is how a
 * screen makes somebody dread a change they have already made up their mind
 * about.
 */
export class HeadUsageDto {
  @ApiProperty() costingId!: number;
  @ApiProperty() eventTypeId!: number;
  @ApiProperty() eventTypeName!: string;
  @ApiProperty({ nullable: true }) slotLabel!: string | null;
  @ApiProperty({ description: 'Which instances this costing speaks for' })
  scope!: string;
  @ApiProperty() effectiveFrom!: string;
  @ApiProperty({ description: 'What this costing pays on that head today' })
  amount!: number;
  @ApiProperty({ nullable: true }) partyName!: string | null;
  @ApiProperty({
    description: 'Itemised heads are decided by their items, so they are not changed here',
  })
  isItemised!: boolean;
}

export class HeadChangeDto {
  @ApiProperty()
  @Type(() => Number)
  @IsInt()
  @Min(1)
...
/** One head, many costings, one act. */
export class UpdateHeadDto {
  @ApiProperty({ description: 'The expense head being repriced' })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  accountId!: number;

  @ApiProperty({ type: () => [HeadChangeDto] })
  @IsArray()
  @ArrayMaxSize(200)
  @ValidateNested({ each: true })
  @Type(() => HeadChangeDto)
  changes!: HeadChangeDto[];
}

export class QueryHeadUsageDto {
  @ApiProperty({ description: 'The expense head to look for' })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  accountId!: number;

  @ApiPropertyOptional({ description: 'Narrow to one pooja type' })
  @IsOptional()
  @Type(() => Number)
  @IsInt()
  @Min(1)
